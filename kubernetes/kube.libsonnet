...
local environment = std.extVar('environment');
{
  ...
  */
  ...
  // which serves the merged pilot-agent/Envoy scrape). The waypoint pod itself
  // is generated by Istio from the Gateway resource, not defined in this
  // library, so target_pod below is a synthetic stand-in with just enough
  // shape (labels.name, a port named 'metrics') for PodMonitor's existing
  // port/selector auto-detection to work unmodified. Exposed standalone (in
  // addition to being bundled into WaypointProxyConfig) for callers that need
  // just the PodMonitor.
